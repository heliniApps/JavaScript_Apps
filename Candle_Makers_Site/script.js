    ...
    return `
        ...
        <ul class="candle-features">
            <li class="candle-prop candle-scent-type">Scent: <span>${
                candleObj.scentType.description
            }</span></li>
...
const applyAlternatingBgColor = (articleElem, candleObj) => {
    ...
    articleElem.querySelectorAll(cssSelector).forEach((listNode) => {
        count++;
        if (count % 2 !== 0) {
            listNode.style.backgroundColor = getListItemBgColor(
                candleObj.color.values
            );
        } else {
            listNode.style.backgroundColor = getListItemBgColor(null);
        }
    });
};
